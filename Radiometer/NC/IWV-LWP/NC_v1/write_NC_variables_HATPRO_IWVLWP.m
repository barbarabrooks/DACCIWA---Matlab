function write_NC_variables_HATPRO_IWVLWP(fn_nc, data)
%time
        ts.Name='time';
        ts.Nctype='double';
...
%integrated_water_vapour
        ...
        as(4).Name='valid_min';as(4).Value=min(data.IWV);
        as(5).Name='valid_max';as(5).Value=max(data.IWV);
...
%liquid_water_path
        ...
        as(3).Name='_FillValue';as(3).Value=-1e+020;
        as(4).Name='valid_min';as(4).Value=min(data.LWP);
        as(5).Name='valid_max';as(5).Value=max(data.LWP);
        as(6).Name='cell_methods';as(6).Value='time: mean';
        as(7).Name='coordinates';as(7).Value='latitude longitude';
        ts.Attribute=as;
        nc_addvar(fn_nc,ts);clear ts as dc
