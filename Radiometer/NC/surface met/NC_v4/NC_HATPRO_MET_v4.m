%function NC_HATPRO_MET_v4(dn,do,fn_meta, platform, version, offset, flag_moving)

fn_meta='G:\renfrew\radiometer\meta_MET_v0.txt';
do='G:\renfrew\radiometer\NC\MET\v0'
...
for n=5:length(list)
    ...
    if ~isempty(data.MET)
        ...
        write_NC_header_v1(fc, meta, data.MET.ET, data.MET.lat, data.MET.lon, flag_moving); 
        %Dimensions - General
        write_NC_dimensions_v1(fc, length(data.MET.ET), length(data.MET.lat), length(data.MET.lon),  flag_moving);        
        %Variables - General - Time
        write_NC_time_v1(fc, data.MET.ET, data.MET.DoY, data.MET.DT);
        %Variables - General - Position
        write_NC_position_v1(fc, data.MET.lat, data.MET.lon, flag_moving);
        %only required when instrument on moving platform
        if flag_moving == 1
            %Variables - General - Motion (can be remmed out if not needed
            write_NC_motion_v1(fc, data.MET.course_true, data.MET.orientation, data.MET.speed_wr_water, data.MET.speed_wr_ground,...
                data.MET.RPY, data.MET.rate_RPY, data.MET.flag_angle, data.MET.flag_rate, flag_moving);
        end
        
        write_NC_MET_header_v4(fc);
        write_NC_MET_dimensions_v4(fc);
        write_NC_MET_variables_v4(fc, data.MET.PP, data.MET.T, data.MET.RH, offset);
        write_NC_MET_qc_flags_v4(fc, data.MET.flag);
    end
