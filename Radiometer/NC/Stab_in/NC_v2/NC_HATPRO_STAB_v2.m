function NC_HATPRO_STAB_v2(dn,do,fn_meta)
%dn='F:\WORKING\Radiometer\matlab';
%do='F:\';
%fn_meta='C:\Users\lecbjb\Desktop\Data Project\Stationary\ncas-scanning-radiometer-1\stability-indices\Example META Data File\Meta data.csv';
...
for n=3:length(list)
    ...
    if ~isempty(data.STA)
        ...
        nc_create_empty(fc);
        %global attributes
        write_NC_header_HATPRO_STAB_v2(fc,meta,data);   
        %Dimensions
        write_NC_dimensions_HATPRO_STAB_v2(fc,meta,data);        
        %Variables
        write_NC_variables_HATPRO_STAB_v2(fc,data);
        %Data    
        write_NC_data_HATPRO_STAB_v2(fc,meta,data);
    end
