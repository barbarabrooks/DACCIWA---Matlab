function write_NC_HATPRO_STAB(fn_nc,fn_data,fn_meta)
...
if ~isempty(data.STA)
    ...
    meta=get_meta_HATPRO_STAB(fn_meta);

    %create nc file, assumes NC_CLOBBER
        nc_create_empty(fn_nc);
    %global attributes
        write_NC_header_HATPRO_STAB(fn_nc,meta,data);   
    %Dimensions
        write_NC_dimensions_HATPRO_STAB(fn_nc,meta,data);        
    %Variables
        write_NC_variables_HATPRO_STAB(fn_nc,data);
    %Data    
        write_NC_data_HATPRO_STAB(fn_nc,meta,data);   
end