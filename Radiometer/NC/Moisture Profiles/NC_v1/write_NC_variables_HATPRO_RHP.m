function write_NC_variables_HATPRO_RHP(fn_nc, data, meta)
%time
        ts.Name='time';
        ts.Nctype='double';
...
%altitude
        ...
        as(6).Name='valid_min';as(6).Value=min(data.Z(1,:))+x;
        as(7).Name='valid_max';as(7).Value=max(data.Z(1,:))+x;
        ts.Attribute=as;
        nc_addvar(fn_nc,ts);clear ts as dc 
%latitude
        ts.Name='latitude';
...
%relative_humidity
        ...
        as(4).Name='_FillValue';as(4).Value=-1e+020;
        as(5).Name='valid_min';as(5).Value=min(min(data.rh));
        as(6).Name='valid_max';as(6).Value=max(max(data.rh));
...
%absolute_humidity
        ...
        as(4).Name='valid_min';as(4).Value=min(min(data.abh));
        as(5).Name='valid_max';as(5).Value=max(max(data.abh));
