function write_NC_HATPRO_RHP(fn_nc,fn_data,fn_meta)
...
if ~isempty(data.HPC)
    ...
    meta=get_meta_HATPRO_RHP(fn_meta);

    %create nc file, assumes NC_CLOBBER
        nc_create_empty(fn_nc);
    %global attributes
        write_NC_header_HATPRO_RHP(fn_nc,meta,data);   
    %Dimensions
        write_NC_dimensions_HATPRO_RHP(fn_nc,meta,data);        
    %Variables
        write_NC_variables_HATPRO_RHP(fn_nc,data,meta);
    %Data    
        write_NC_data_HATPRO_RHP(fn_nc,meta,data);   
end