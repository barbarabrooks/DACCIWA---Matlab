%function NC_HATPRO_MOIST_v4(dn,do,fn_meta, platform, version, offset, flag_moving)

fn_meta='G:\renfrew\radiometer\meta_MOIST_v1.txt';
do='G:\renfrew\radiometer\NC\Moisture\v1'
...
data_product = 'moisture-profiles';
...
for n=5:length(list)
    ...
    if ~isempty(data.HPC)
        ...
        write_NC_header_v1(fc, meta, data.HPC.ET, data.HPC.lat, data.HPC.lon, flag_moving); 
        %Dimensions - General
        write_NC_dimensions_v1(fc, length(data.HPC.ET), length(data.HPC.lat), length(data.HPC.lon),  flag_moving);        
        %Variables - General - Time
        write_NC_time_v1(fc, data.HPC.ET, data.HPC.DoY, data.HPC.DT);
        %Variables - General - Position
        write_NC_position_v1(fc, data.HPC.lat, data.HPC.lon, flag_moving);
        %only required when instrument on moving platform
        if flag_moving == 1
            %Variables - General - Motion (can be remmed out if not needed
            write_NC_motion_v1(fc, data.HPC.course_true, data.HPC.orientation, data.HPC.speed_wr_water, data.HPC.speed_wr_ground,...
                data.HPC.RPY, data.HPC.rate_RPY, data.HPC.flag_angle, data.HPC.flag_rate, flag_moving);
        end
        
        write_NC_MOIST_header_v4(fc);
        write_NC_MOIST_dimensions_v4(length(data.HPC.Z),fc);
        write_NC_MOIST_variables_v4(fc, data.HPC, offset);
        write_NC_MOIST_qc_flags_v4(fc, data.HPC.flag);
    end
