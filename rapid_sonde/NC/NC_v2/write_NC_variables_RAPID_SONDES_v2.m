function write_NC_variables_RAPID_SONDES_v2(fn_nc, data)
%time
        ts.Name='time';
        ts.Nctype='double';
        ...
        as(5).Name='valid_min';as(5).Value=min(data.ST);
        as(6).Name='valid_max';as(6).Value=max(data.ST);
...
%alt_time
        ...
        as(4).Name='valid_min';as(4).Value=min(data.ET);
        as(5).Name='valid_max';as(5).Value=max(data.ET);
        ts.Attribute=as;
        nc_addvar(fn_nc,ts);clear ts as dc
%day_of_year
        ts.Name='day_of_year';
