function NC_Wrapper_RAPID_SONDES(dn,do,fn_meta)
...
for n=3:length(list)
    temp=list(n).name;
    ix=strfind(temp,'_');
    f3=[temp(ix-8:ix-1),'-',temp(ix+1:ix+6),'_'];
    clear temp
    fc=[f1,f2,f3,f4,f5,f6];
    write_NC_SONDES(fullfile(do,fc),fullfile(dn,list(n).name),fn_meta);
    clear fc temp ix f3
end
