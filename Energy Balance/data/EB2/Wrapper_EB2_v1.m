function Wrapper_EB2_v1(dn,do)

%dn='E:\raw\fluxtower1a\sorted';
%do='E:\WORKING\EB1\matlab';
HK=[];RD=[];GPS=[];SHF=[];SM=[];ST=[];
% parse the data    
[HK] =get_EB2_HouseKeeping_v1(fullfile(dn,'CR3000_Housekeeping.dat'));
%[RD] =get_EB2_Radiation_v1(fullfile(dn,'CR3000_Radiation.dat'));
%[GPS] =get_EB2_Radiation_v1(fullfile(dn,'CR3000_GPS_datetime.dat'));
[SHF]=get_EB2_SoilHeatFlux_v1(fullfile(dn,'CR3000_SoilHeatFlux.dat'));
[SM] =get_EB2_SoilMoisture_v1(fullfile(dn,'CR3000_SoilMoisture.dat'));
[ST] =get_EB2_SoilTemperature_v1(fullfile(dn,'CR3000_SoilTemperature.dat'));
...
for n=1:12
    for m=1:31
        data.HK=[];data.RD=[];data.GPS=[];data.SHF=[];data.SM=[];data.ST=[];
