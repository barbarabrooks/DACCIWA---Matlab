function NC_METEK_winds_v2(dn,do,fn_meta)
%dn='F:\WORKING\Flux1\QC_data\1_min\matlab';
%do='F:\';
%fn_meta='C:\Users\lecbjb\Desktop\Data Project\Stationary\ncas-sonic-2\Example META Data File\Meta data.csv';
...
for n=3:length(list)
    ...
    %local code
    [DoY, ST, ET]=file_times(data.means.DT');
    data.means.DoY=DoY';data.means.ST=ST';data.means.ET=ET';
    
    %create nc file, assumes NC_CLOBBER
    nc_create_empty(fc);
    %global attributes
    write_NC_header_METEK_winds_v2(fc,meta,data);   
    %Dimensions
    write_NC_dimensions_METEK_winds_v2(fc,meta,data);        
    %Variables
    write_NC_variables_METEK_winds_v2(fc,data);
    %Data    
    write_NC_data_METEK_winds_v2(fc,meta,data);
