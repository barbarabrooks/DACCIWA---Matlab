function create_daily_files_v2(dn, do)
...
for n=st_month:ed_month
    for m=1:eomday(2000+str2num(year),n)
        ...
        if ~isempty(file_list)
            ...
            licor.DT=[];licor.T=[];licor.PP=[];licor.CO2=[];licor.H2O=[];
            for nn=1:length(file_list)
                load(fullfile(dn,list(file_list(nn)).name));
                display(['nn=',num2str(nn),' ',list(file_list(nn)).name])
            ...
            if ~isempty(ix)
                data.raw.sonic.DT=sonic.DT(ix,:);
                data.raw.sonic.xyz=sonic.xyz(ix,:);
                data.raw.sonic.T=sonic.T(ix,1);        
            end
            clear ix
            ix=find((licor.DT(:,2)==n) & (licor.DT(:,3)==m));
            if ~isempty(ix)
                data.raw.licor.DT=licor.DT(ix,:);
                data.raw.licor.T=licor.T(ix,1);
                data.raw.licor.PP=licor.PP(ix,1);
                data.raw.licor.CO2=licor.CO2(ix,:);
                data.raw.licor.H2O=licor.H2O(ix,:);
            end
            if ((~isempty(data.raw.sonic.DT))|(~isempty(data.raw.licor.DT)))
                fo=['20',year];
                if n<10
                    fo=[fo,'0',num2str(n)];
                else
