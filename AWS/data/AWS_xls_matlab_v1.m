function AWS_xls_matlab_v1(di,dn_xls,dn_mat)
%input
%   di='E:\WORKING\AWS\DB_CSV_dump\AWS_data.csv';
%       the raw dump from Davis station sql data base
%       this is imperial so function converts to metric

%output
%   dn_xls='E:\WORKING\AWS\xls';
%       directory for the xls spread sheet
%   dn_mat='E:\WORKING\AWS\matlab\Initial'
%       directory for matlab data structure
...
for n = 3:length(list)
    dd=xlsread(fullfile(di,list(n).name));
    display(list(n).name)
    ...
    for m=6:7
        for n=1:31
            ...
            if ~isempty(ix)
                ...
                save(fullfile(dn_mat,fn_mat),'data','-v7.3');
