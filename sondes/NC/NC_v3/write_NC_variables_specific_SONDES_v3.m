function write_NC_variables_specific_SONDES_v3(fn_nc, data)
%altitude
        ts.Name='altitude';
        ts.Nctype='float32';
        ...
        as(6).Name='valid_min';as(6).Value=min(data.ZZ);
        as(7).Name='valid_max';as(7).Value=max(data.ZZ);
...
%air_pressure
        ...
        as(4).Name='_FillValue';as(4).Value=-1e+020;
        as(5).Name='valid_min';as(5).Value=min(data.PP);
        as(6).Name='valid_max';as(6).Value=max(data.PP);
...
%air_temperature
        ...
        as(4).Name='_FillValue';as(4).Value=-1e+020;
        as(5).Name='valid_min';as(5).Value=min(data.TT);
        as(6).Name='valid_max';as(6).Value=max(data.TT);
        as(7).Name='cell_methods';as(7).Value='time: point';
        as(8).Name='coordinates';as(8).Value='latitude longitude';
        ts.Attribute=as;
        nc_addvar(fn_nc,ts);clear ts as dc        
...
%elapsed_time
        ...
        as(4).Name='valid_min';as(4).Value=min(data.EP);
        as(5).Name='valid_max';as(5).Value=max(data.EP);
        as(6).Name='cell_methods';as(6).Value='time: point';
        ts.Attribute=as;
        nc_addvar(fn_nc,ts);clear ts as dc        
